import type { TimeBlock, BlockType, UserPattern } from '../types';
...
export class TimelineService {
  ...
  async setUserPattern(pattern: UserPattern): Promise<void> {
  ...
  private async findNextAvailableSlot(task: Task): Promise<Date> {
    if (!this.userPattern) {
      throw new Error('User pattern not set');
    }
  ...
  async suggestBreaks(start: Date, end: Date): Promise<TimeBlock[]> {
    if (!this.userPattern) {
      throw new Error('User pattern not set');
    }

    const blocks = await this.timelineManager.getBlocksInRange(start, end);
    const suggestedBreaks: TimeBlock[] = [];
    ...
    while (currentTime < end) {
      ...
      if (gap >= breakDuration) {
        ...
        const breakStart = new Date(
          currentTime.getTime() + (gap - breakDuration) / 2 * 60 * 1000
        );
  ...
  async getTimelineManager(): Promise<TimelineManager> {
